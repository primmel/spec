...
(<<term-alias-family,13.10.1>>): a bare string entry of an `aliases`,
`colloquial`, `abbreviations`, or `deprecated` list has no declared key
...
are authored in `text` blocks and never inline.
