...
|Reference implementation |`@primmel/primmel` (primmel-ts) 1.9.0
|===

Revision note (2026-09-05). This revision of the draft removes the
*roadmap* marks of the v3.2 extension set: the condition of the
...
`@primmel/primmel` 1.9.0, and the standalone conformance suite at
...
calculation-signature example of <<calculation-signatures,13.7.1>> now
shows the entry form the toolchain ships (the v2
`<name> : <type> { ... }` spelling), and the three rollout legs the
...
(oimlsmart/smart#256) declares R 144's `capabilities` axis as a
...
(<<composition-rules,7.6>>, checker rule C119): a requirement scope a
...
the rule at `@primmel/primmel` 1.10.0; the conformance suite at version
...
<<term-alias-family,13.10.1>>), the typed applicability dimension
(the top-level `dimension` construct and the applicability namespace,
<<dimension-construct,10.6>>, <<applicability-semantics,11.1.1>>), the
...
forward lineage edge (manifest `superseded_by` and the edition lineage
graph, <<edition-lineage,9.3.1>>), and units-typed quantities (the
calculation signature's `quantity_kind` + `range` facets and the limit's
`quantity { kind unit }` block, <<calculation-signatures,13.7.1>>,
<<limit-quantities,11.1.2>>), and the verdict chain with the
instance-parameter schema (verdict `inputs` may name verdicts;
requirement parameters carry `unit` and `bind`,
